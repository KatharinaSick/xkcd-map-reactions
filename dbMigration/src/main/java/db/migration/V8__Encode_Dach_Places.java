...
/**
 * Encodes all stored dach_places with the Beider-Morse and the Nysiis Phonetic Matching Algorithm. Then it stores
 * all codes and their matching place ids into new databases.
 */
public class V8__Encode_Dach_Places extends BaseJavaMigration {
    ...
    public void migrate(Context context) throws Exception {
        ...
        String beiderMorseInsertStatement = "INSERT INTO beider_morse_encoded_dach_places (code, place_id) VALUES  (?,?)";
        PreparedStatement beiderMorsePreparedStatement = context.getConnection().prepareStatement(beiderMorseInsertStatement);

        String nysiisInsertStatement = "INSERT INTO nysiis_encoded_dach_places (code, place_id) VALUES  (?,?)";
        PreparedStatement nysiisPreparedStatement = context.getConnection().prepareStatement(nysiisInsertStatement);

        String soundexInsertStatement = "INSERT INTO soundex_encoded_dach_places (code, place_id) VALUES  (?,?)";
        ...
        try (Statement select = context.getConnection().createStatement()) {
            try (ResultSet rows = select.executeQuery("SELECT id, name FROM dach_places")) {
